...
class PaymentMethodQueryService
{
    ...
    public function getDriver(PaymentData|SalesPaymentData $data): PaymentDriverInterface
    {
        return collect($this->drivers)
            ->first(fn (PaymentDriverInterface $payment) => $payment->driver == $data->driver);
    }

    public function getPaymentMethods(): DataCollection
    {
        return collect($this->drivers)
            ->flatMap(fn (PaymentDriverInterface $interface) => $interface->getMethods()->toCollection())
